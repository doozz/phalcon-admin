...
class ImgUpload
{
    ...
    */
    public function __construct($di, $allow_types = '')
    {
        ...
        $this->max_size = $this->di['config']['allowedFileSize'];
    }

    public function upload($files, $path, $goodsId = 0, $condtion = 0, $judge = 1, $type = '')
    {
        //time 命名
        if($condtion == 0 ){
            //获取文件后缀名
            ...
            $data = substr(md5($type.$goodsId), 8, 16);
            $fullFileName= $data . $type;
            $repath = rtrim($this->setDir($data), '/');
        }

        $this->save_path =__DIR__.'/../../' .ltrim($path, '/') .'/'. $repath;
      
        //判断是否需要创建文件夹, judge=0只做判断文件合法性
            if($judge == 1 ){
        ...
        if($files['error'] == 0 && is_uploaded_file($files['tmp_name']))
        {
            ...
            {
                $this->errmsg = '上传文件'.$files['name'].'太大，最大支持'.ceil($this->max_size/1024).'kb的文件';
                return FALSE;
            }
            // 上传原图
            ...
            if($judge == 1 ){
                if (move_uploaded_file($files['tmp_name'], $fullPath)) {
                    $type = explode('/', $path);
                    return $type[1] .$type[2] . '/' . $repath . '/' . $fullFileName;
                } else {
                    $this->errmsg = '文件上传失败！';
                    return FALSE;
                }
